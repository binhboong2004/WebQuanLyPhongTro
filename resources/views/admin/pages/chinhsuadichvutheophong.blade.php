...
@section('title', 'Chỉnh sửa dịch vụ phòng')
...
<main class="flex-1 p-8">
    ...
    <form action="{{ route('services.rooms.update', $service->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-6 rounded-2xl shadow-sm">
            ...
            <div>
                ...
                <select name="room_id" class="search-input w-full border p-2 rounded-xl">
                    ...
                    @foreach($rooms as $r)
                    <option value="{{ $r->id }}" {{ $service->room_id == $r->id ? 'selected' : '' }}>{{ optional($r->building)->name ? optional($r->building)->name . ' / ' : '' }}{{ $r->room_number }}</option>
                    @endforeach
                </select>
            </div>
