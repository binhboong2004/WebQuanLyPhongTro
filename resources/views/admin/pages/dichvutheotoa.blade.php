...
@section('title', 'Dịch vụ theo tòa nhà')

@section('content')
<main class="flex-1 p-8">
    <div class="flex justify-between items-center mb-6">
        <h3 class="font-black text-gray-700">Dịch vụ theo tòa nhà</h3>
        <a href="{{ route('services.buildings.create') }}" class="btn-primary">Thêm dịch vụ tòa</a>
    </div>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="bg-white divide-y">
                    ...
                    <tr>
                        ...
                        <td class="p-4">{{ optional($s->building)->name ?? '—' }}</td>
                        <td class="p-4 text-right">{{ number_format($s->price_per_unit,0,',','.') }}đ</td>
                        <td class="p-4 text-center">
                            <div class="flex justify-center gap-2">
                                <a href="{{ route('services.buildings.edit', $s->id) }}" class="action-btn"><i class="fa fa-pen"></i></a>
                                <form action="{{ route('services.buildings.destroy', $s->id) }}" method="POST" onsubmit="return confirm('Xóa dịch vụ này?')">
                                    @csrf
                                    @method('DELETE')
